...
class EnvBase(gym.Env):
    def __init__(self, datapath="data/sensor_data_with_anomalylabel_isolationforest.csv", budget=100):
        # Read dataset
        ...
        high = np.ones(self.state_dim, dtype=np.float32) * 10.0
        low = np.ones(self.state_dim, dtype=np.float32) * -10.0
...
class EvalEnv(EnvBase):
    ...
    def step(self, action):
        ...
        # budget이 None이면 모든 데이터를 탐색, 아니면 budget만큼만 탐색
        if self.budget is None:
            # 모든 legal action이 없어지면 종료 (전체 데이터셋 탐색 완료)
            if len(self.legal) == 0:
                self.done = True
            else:
                self.done = False
